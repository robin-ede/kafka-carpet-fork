...
public class HopperCounter
{
    ...
    public static void onItemPassThroughHopper(DyeColor woolColor, ItemStack itemStack) {
        // Get item name and count from the hopper
        String itemName = itemStack.getItem().toString();
        int itemCount = itemStack.getCount();

        // Get the current game tick (overworld game time in ticks)
        MinecraftServer server = CarpetServer.minecraft_server;
        long gameTick = (server != null) ? server.getTickCount() : -1;

        // Format the data to be sent to Kafka
        String message = String.format(
                "WoolColor: %s, ItemName: %s, ItemCount: %d, GameTick: %d",
                woolColor.getName(),
                itemName,
                itemCount,
                gameTick
        ...
        KafkaHelper.sendMessage("item_event", message);
    }
}
